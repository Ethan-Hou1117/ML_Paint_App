...
class Paint(object):
    ...
    def __init__(self):
        ...
        self.pen_button = Button(self.root, text='pen', command=self.paint)
        self.pen_button.grid(row=0, column=0)

        self.capture_pic_button = Button(self.root, text='screenshot', command=self.save_image)
        self.capture_pic_button.grid(row=0, column=1)
    ...
    def save_image(self):
        # Save the canvas drawing to a temporary file
        file = io.BytesIO()
        self.c.postscript(file=file, colormode='color')
        file.seek(0)
        ...
        response = requests.post(server_url, files={"file": ("image.png", img_file)})
        print(response)
